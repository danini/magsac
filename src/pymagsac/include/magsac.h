	...
	for (size_t iterations = 0; iterations < number_of_irwls_iters; ++iterations)
	{
		...
		for (const auto &[residual, idx] : residuals)
		{
			...
			else
			{
				...
				// Look up the upper incomplete gamma Γ((DoF-1)/2, x) per DoF
				double gamma_at_x;
				if constexpr (degrees_of_freedom == 1)
				{
					if (stored_gamma_number_dof1 < x) x = stored_gamma_number_dof1;
					gamma_at_x = stored_gamma_values_dof1[x];
				}
				else if constexpr (degrees_of_freedom == 2)
				{
					// Γ(0.5, t) = √π · erfc(√t), where t = x * (1/1000)
					gamma_at_x = 1.7724538509055159 * std::erfc(std::sqrt(static_cast<double>(x) * 0.001));
				}
				else if constexpr (degrees_of_freedom == 3)
				{
					// Γ(1, t) = e^(-t)
					gamma_at_x = std::exp(-static_cast<double>(x) * 0.001);
				}
				else if constexpr (degrees_of_freedom == 4)
				{
					if (stored_gamma_number < x) x = stored_gamma_number;
					gamma_at_x = stored_gamma_values[x];
				}
				else
				{
