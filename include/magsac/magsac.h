	...
	const ModelEstimator &estimator_)
{
	...
	std::vector<pair<double, int>> all_residuals;
	...
	const auto comparator = [](pair<double, int> left, pair<double, int> right) { return left.first < right.first; };
	...
	const ModelScore &best_score_)
{
	...
	static auto cmp = [](pair<double, int> left, pair<double, int> right) { return left.first < right.first; };
	...
	concurrency::parallel_for(0, static_cast<int>(core_number), [&](int process)
	{
		...
		{
			const pair<double, int> &next = all_residuals[sigma_idx];
			const double sigma = next.first;
	...
	double &score_)
{
	...
	const int M = estimator_.sampleSize();
	...
	for (auto pt_idx = 0; pt_idx < Ni; ++pt_idx)
	{
		...
		for (int i = 0; i < partition_number - 1; ++i)
		{
			if (ri < thresholds[i])
			{
				...
				probabilities[i] += exp(-ri2 /  thresholds_2_sqr[i]);
			}
		}
	}
