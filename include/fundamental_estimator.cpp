...
public:
		...
		std::vector<FundamentalMatrix>* models) const
	{
		...
		cv::Mat normalized_points(data.size(), data.type());
		cv::Mat T1, T2;
		...
		cv::Mat &T2) const
	{
		...
		T1 = (cv::Mat_<double>(3, 3) << ratio1, 0, -ratio1 * mass1[0],
			0, ratio1, -ratio1 * mass1[1],
			0, 0, 1);

		T2 = (cv::Mat_<double>(3, 3) << ratio2, 0, -ratio2 * mass2[0],
